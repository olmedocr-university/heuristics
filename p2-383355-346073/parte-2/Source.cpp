	...
	if (s.buspos.type == "school") {
		for (int i = 0; i < (int)s.students.size(); i++) {
			...
			if (s.buspos.schoolName == s.students[i]) {
				...
				vector<vector<string>> temp = newState.studentsInNodes[nodePos(newState.buspos.name, newState.nodeList)];
				bool inserted = false;
				...
				for (int k = 0; k < (int)temp.size(); k++) {
					...
					if (temp[k][1] == s.students[i]) {
						...
						newState.studentsInNodes[nodePos(newState.buspos.name, newState.nodeList)][k][0] = to_string(num);
						break;
					}
				}
					...
					if (newState.studentsInNodes[nodep].size() > 0) {
						...
						for (int j = 0; j < (int)newState.studentsInNodes[nodep].size(); j++) {
							...
							if (newState.studentsInNodes[nodep][j][1] >= stud[1]) {
								...
								newState.studentsInNodes[nodep].insert(newState.studentsInNodes[nodep].begin() + j, stud);
								break;
...
bool equalStates(state s1,state s2) {
	...
	for (int i = 0; i < (int)s1.studentsInNodes.size(); i++) {
		...
		for (int k = 0; k < (int)s1.studentsInNodes[i].size(); k++) {
			if (s1.studentsInNodes[i][k][0] != s2.studentsInNodes[i][k][0] || s1.studentsInNodes[i][k][1] != s2.studentsInNodes[i][k][1]) {
				return false;
			}
		}
	}
